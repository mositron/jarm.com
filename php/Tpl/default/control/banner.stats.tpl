...
<tr><td style="width:120px; text-align:right">ตำแหน่ง</td><td>
<?php if($this->banner['p']):?>
<?php foreach($this->banner['p'] as $k=>$v):?>
<div>
<strong><?php echo $this->position[$k]['t']?></strong><br>
<?php for($i=0;$i<count($v);$i++):?>
<?php echo $i?', ':''?><?php echo strtoupper($v[$i])?>
<?php endfor?>
</div>
<?php endforeach?>
<?php endif?>
...
<tr><td style="width:120px; text-align:right">เวลาในการแสดงผล</td><td><?php echo self::Time()->from($this->banner['dt1'],'date')?> - <?php echo self::Time()->from($this->banner['dt2'],'date')?></td></tr>
...
<script type="text/javascript" src="<?php echo FILES_CDN?>js/flot/jquery.flot.js"></script>
<script>
