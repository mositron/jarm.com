...
<a href="/cooked" class="icon-exit"></a>
...
var m={
	...
	open:function()
	{
		...
		{
			tmp+='<div onclick="m.opencard('+i+');"><p><img src="<?php echo FILES_CDN?>img/mobile/cooked/'+m.game.icon+'/'+m.card[i]+'.png"></p><div id="cell_'+i+'"></div></div>';
		}
		$('#user-score').html(m.score-m.fail);
		$('#user-fail').html('');
		$('#progress').css('display','block');
		$('#waiting').css('display','none');
		$('#cooked').css('display','block').html(tmp);
	...
	nextlevel:function(play,user)
	{
		...
		if(play)
		{
			tmp+='<div class="msg-box"><h3>สำเร็จ!.</h3><div>ผ่านเกมเลเวล <?php echo $this->lv?> แล้ว...<br><br>'+tmp2+'<div id="btn-no-fb"><a href="/cooked/game/<?php echo $this->user['_id']?>/'+user.lv+'" class="btn btn-next">เกมถัดไป (Lv.'+user.lv+')</a><a href="/cooked/game/<?php echo $this->user['_id']?>/<?php echo $this->lv?>" class="btn">เล่นเกมนี้ซ้ำอีกครั้ง</a><a href="/cooked/score/<?php echo $this->user['_id']?>" class="btn">ไปหน้าคะแนนสะสม</a></div></div></div>';
		}
		else
		{
			tmp+='<div class="msg-box"><h3>เลเวลสูงสุดแล้ว.</h3><div>ขณะนี้เกมจับคู่จำกัดเลเวลสูงสุดที่ <?php echo $this->maxlv?>. กรุณารออัพเดทเลเวลใหม่เร็วๆนี้.<br><br>'+tmp2+'<div id="btn-no-fb"><a href="/cooked/score/<?php echo $this->user['_id']?>" class="btn">ไปหน้าคะแนนสะสม</a><a href="/cooked" class="btn btn-exit">ออกจากเกม</a></div></div></div>';
		}
	...
	playagain:function(play)
	{
		...
		{
			tmp+='<div class="msg-box"><h3>เลเวลสูงสุดแล้ว.</h3><div>ขณะนี้เกมจับคู่จำกัดเลเวลสูงสุดที่ <?php echo $this->maxlv?>. กรุณารออัพเดทเลเวลใหม่เร็วๆนี้.<br><br><a href="/cooked" class="btn btn-exit">ออกจากเกม</a></div></div>';
		}
		$('#preload').html(tmp).css('display','block');
	},
	...
	resize:function()
	{
		...
		$('#cooked>div').css({'margin':ml+'px 0px 0px 1%','height':bh,'width':bw});
		$('#cooked>div>div,#cooked>div>p').css({'height':bh-2,'width':bw});
