...
#$cache=Load::$core;
#if(!Load::$core->data['content']=$cache->get('ca1','football_next_match'))
#{
	$db=Load::DB();
	$match=[];
	...
	$d1=$d2+(86400*30);
	if($dd=$db->distinct('football_match','ky',array('tm'=>array('$gte'=>Load::Time()->from($d2),'$lt'=>Load::Time()->from($d1)))))
	{
		sort($dd);
	...
	Load::$core->data['content']=Load::$core->fetch('football.next-match');


#	$cache->set('ca1','football_next_match',Load::$core->data['content'],false,300);
#}
?>
