...
<?php foreach($this->allorder as $key=>$val):?>
<?php if($key=='p'):?>
<td width="60" class="tab_<?php echo $key?>"><?php if($this->app[$i]['img']):?><img src="<?php echo self::uri(['s3','/sticker/cover/'.$this->app[$i]['fd'].'/s.png'])?>" style="width:60px"><?php endif?></td>
<?php elseif($key=='s'):?>
...
<?php if($this->app[$i]['pl']):?>
<br><a href="<?php echo self::uri(['sticker','/view/'.$this->app[$i]['_id']])?>" target="_blank"><?php echo self::uri(['sticker','/view/'.$this->app[$i]['_id']])?></a>
<?php endif?>
