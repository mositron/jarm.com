...
Load::$core->data['content']=Load::$core->fetch('hidden.game');
...
function setpass($arg)
{
	...
	$game=require(__DIR__.'/mobile.hidden.game.config.php');
	if(isset($game[$arg['lv']]))
	{
		...
		if($sc==$max && $fail<$max && USER_ID==$arg['id'] && USER_FB==$arg['fb'])
		{
			...
			if(count($set)>0)
			{
				...
				$db->update('hidden_user',['_id'=>USER_ID],['$set'=>$set]);
			}
			$fb=array(
								'message'=>'อัพเลเวล '.$nlv.'. ใน เกมจับผิด+',
								'name'=>'เลเวล '.$nlv.'!.',
								'caption'=>'อัพเลเวล '.$nlv.'. ใน เกมจับผิด+ สำหรับ Android',
								'link'=>'https://play.google.com/store/apps/details?id=com.doodroid.hidden',
								'picture'=>'https://lh6.ggpht.com/3qgcgzMX5TmSq6kWthzd9IwhA7O62k5jfHY0swhyjwqCqSJ3FUbsoFqjmuu1APFAyQ',
								'description'=>'เกมจับผิด+ by jarm.com - เกมทดสอบควมจำ เก็บเลเวล สะสมแต้ม เล่นง่ายๆ บนมือถือ/แท็บเล็ต Android',
								'actions'=>[['name'=>'เกมจับผิด+ for Android','link'=>'https://play.google.com/store/apps/details?id=com.doodroid.hidden']]
			);
			if($wall)
			{
