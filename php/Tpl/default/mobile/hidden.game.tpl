<div id="user-bar">
<div>Lv. <span id="user-lv"><?php echo $this->lv?></span></div>
<div>คะแนนสะสม <span id="user-score">0</span> <span id="user-fail"></span></div>
<p>
<a href="/hidden">ออกจากเกม</a>
...
<p>เหลือเวลา <span id="time-remain"><?php echo $this->game['time']?></span> วินาที</p>
</div>
...
<div id="hidden_warp">
<div id="hidden">
<div id="hidden_left"><p style="font-size:36px; text-align:center; line-height:100%">1</p></div>
<div id="hidden_right"><p style="font-size:36px; text-align:center; line-height:100%">2</p></div>
</div>
</div>
<div id="preload"></div>
...
<script>
var m={
	user:<?php echo json_encode($this->user)?>,game:<?php echo json_encode($this->game)?>,ctime:60,tmr:'',tmrpre:'',card:[],opened:[],state:-2,score:0,fail:0,wall:false,resend:0,
	start:function()
	{
		m.state=-1;
		m.ctime=m.game.time;
	...
	open:function()
	{
		...
		{
			tmp+='<div onclick="m.opencard('+i+');"><img src="<?php echo FILES_CDN?>img/mobile/hidden/'+m.game.icon+'/'+m.card[i]+'.png"><div id="cell_'+i+'"></div></div>';
		}
		$('#user-score').html(m.score);
		$('#user-fail').html('');
		*/
	...
	nextlevel:function(play,user)
	{
		...
		if(play)
		{
			tmp+='<div class="msg-box"><h3>สำเร็จ!.</h3><div>ผ่านเกมเลเวล <?php echo $this->lv?> แล้ว...<br><br>'+tmp2+'<div id="btn-no-fb"><a href="/hidden/game/<?php echo $this->user['_id']?>/'+user.lv+'" class="btn btn-next">เกมถัดไป (Lv.'+user.lv+')</a><a href="/hidden/game/<?php echo $this->user['_id']?>/<?php echo $this->lv?>" class="btn">เล่นเกมนี้ซ้ำอีกครั้ง</a><a href="/hidden/score/<?php echo $this->user['_id']?>" class="btn">ไปหน้าคะแนนสะสม</a></div></div></div>';
		}
		else
		{
			tmp+='<div class="msg-box"><h3>เลเวลสูงสุดแล้ว.</h3><div>ขณะนี้เกมจับผิดจำกัดเลเวลสูงสุดที่ <?php echo $this->maxlv?>. กรุณารออัพเดทเลเวลใหม่เร็วๆนี้.<br><br>'+tmp2+'<div id="btn-no-fb"><a href="/hidden/score/<?php echo $this->user['_id']?>" class="btn">ไปหน้าคะแนนสะสม</a><a href="/hidden" class="btn btn-exit">ออกจากเกม</a></div></div></div>';
		}
	...
	playagain:function(play)
	{
		...
		{
			tmp+='<div class="msg-box"><h3>เลเวลสูงสุดแล้ว.</h3><div>ขณะนี้เกมจับผิดจำกัดเลเวลสูงสุดที่ <?php echo $this->maxlv?>. กรุณารออัพเดทเลเวลใหม่เร็วๆนี้.<br><br><a href="/hidden" class="btn btn-exit">ออกจากเกม</a></div></div>';
		}
		$('#preload').html(tmp).css('display','block');
	},
	...
	resize:function()
	{
		...
		var top=$('#hidden').offset().top,space = h-top;
		var c=48.5;
		var ml=parseInt(w/100);
		var bw=parseInt((w/100)*c);
		var bh=parseInt(space-(ml*2));
		$('#hidden').css({'height':space});
		$('#hidden>div').css({'height':bh,'width':bw,'margin-top':ml});
		$('#hidden>div>div').css({'height':bh,'width':bw});
