...
$pp=50;
extract(Load::Split()->get('/sticker/recent',1,['page']));

$db=Load::DB();
if($count=$db->count('sticker',['pl'=>1,'dd'=>['$exists'=>false]]))
{
	list($pg,$skip)=Load::Pager()->navigation($pp,$count,[$url,'page-'],$page);
	$app=$db->find('sticker',['pl'=>1,'dd'=>['$exists'=>false]],['t'=>1,'fd'=>1,'do'=>1,'f'=>1],['sort'=>['_id'=>-1],'skip'=>$skip,'limit'=>$pp]);
}
...
Load::$core->data['content']=Load::$core->fetch('sticker.recent');
