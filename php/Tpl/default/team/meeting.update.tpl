<ul class="breadcrumb">
  ...
  <span class="divider">&raquo;</span>
  <li><a href="/meeting/insert" title="">แก้ไข</a></li>
</ul>
...
<div class="box-white">
  ...
  <?php endif?>
  <form action="<?php echo URL?>" class="form-horizontal" method="post"  onsubmit="_.ajax.gourl('<?php echo URL?>','updatemeeting',this);return false;">
    <div class="form-group">
      <label for="title" class="col-sm-2 control-label">หัวข้อ</label>
      <div class="col-sm-10">
        <input type="text" name="title" class="form-control" id="title" placeholder="หัวข้อ" value="<?php echo $this->meeting['title']?>"/>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <div class="input-group">
          ...
          <input type="text" name="time" readonly="true" class="form-control pull-right" id="time" value="<?php echo date('Y-m-d H:i:s',self::Time()->sec($this->meeting['dp']))?>" />
        </div>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="col-sm-10">
        ...
        <?php $last=1;foreach($this->people as $k=>$v):?>
          ...
          <?php endif?>
          <option value="<?php echo $k?>"<?php echo in_array($k,(array)$this->meeting['ref'])?' selected':''?>><?php echo $v['nickname'].' - '.$v['th']['first'].' '.$v['th']['last']?></option>
        <?php endforeach?>
          </optgroup>
        </select>
      </div>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <textarea class="mceEditor" id="detail" name="detail" style="height:700px; width:630px;" minlength="20"><?php echo htmlspecialchars($this->meeting['detail'],ENT_QUOTES,"UTF-8")?></textarea>
      </div>
    </div>
    <div class="form-group">
