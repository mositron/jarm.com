...
if($arg['type']=='open')
{
	...
	$status=0;
	$ty=array(-1=>'วัตถุดิบ',0=>'ยา',1=>'อาวุธ',2=>'หมวก',3=>'เสื้อ',4=>'กางเกง',5=>'ถุงมือ',6=>'รองเท้า');
	$job=$this->config('job');
	...
	if($vender=$this->db->findone('lionica_vender',array('u'=>_::$my['_id'])))
	{
		if(is_array($vender['item']))
		{
			...
			foreach($vender['item'] as $vd)
			{
				...
				{
					$left.='<tr><td class="n">'.$vd['name'].'</td><td class="c">'.$vd['count'].'</td><td class="p">'.$vd['price'].'</td></tr>';
				}
			}
			$left.='</table>';	
		}
	...
	for($i=0;$i<count($inv);$i++)
	{
		...
		$item=$items[$inv[$i]['item']];
		$popup='ประเภท: '.$ty[$item['type']];
		$plus_price=0;
		if($inv[$i]['nh'])
		{
			if($item['type']==1)
		...
		if($item['sell'])
		{
			$popup.=', ขาย: '.number_format($item['sell']).($plus_price?' (+'.$plus_price.')':'');	
		}
		$right.='<tr>';
		$right.='<td class="l'.($i%2).' n"><a href="javascript:;" onclick="_.lionica.shop.insert(this,'.$inv[$i]['_id'].','.$inv[$i]['c'].','.$item['sell'].')" class="lionica-popup" data-popup="'.$popup.'"><i class="item" style="background-position:'.$item['css'].'"></i> <span>'.($inv[$i]['nh']?'+'.$inv[$i]['nh'].' ':'').$item['name'].'</span></a></td>';
			...
			if(isset($items[$inv['item']]))
			{
				...
				if($price>0)
				{
					if($count>=1&&$count<=$inv['c'])
					{
						if($vender=$this->db->findone('lionica_vender',array('u'=>_::$my['_id'])))
						{
							$this->db->update('lionica_vender',array('u'=>_::$my['_id'],'p'=>$this->char['_id']),array('$set'=>array('item.i'.$inv['_id']=>array('item'=>$inv['item'],'inv'=>$inv['_id'],'name'=>($inv['nh']?'+'.$inv['nh'].' ':'').$item['name'],'count'=>$count,'price'=>$price),'du'=>new MongoDate())));
						}
						else
						{
							$this->db->insert('lionica_vender',array('u'=>_::$my['_id'],'p'=>$this->char['_id'],'n'=>$this->char['n'],'item'=>array('i'.$inv['_id']=>array('item'=>$inv['item'],'inv'=>$inv['_id'],'name'=>($inv['nh']?'+'.$inv['nh'].' ':'').$item['name'],'count'=>$count,'price'=>$price)),'du'=>new MongoDate()));
						}
		...
		if(is_array($vender['item']))
		{
			...
			foreach($vender['item'] as $vd)
			{
				if(isset($items[$vd['item']]))
				{
					...
					$left.='<tr>
						<td class="l'.($i%2).' n">
						<a href="javascript:;" onclick="_.lionica.shop.buy(this,'.$vd['inv'].','.$vd['count'].','.$vd['price'].','.$vender['_id'].')" class="lionica-popup" data-popup="'.$popup.'">
