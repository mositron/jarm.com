...
<li class="col-sm-4">
<h4><a href="/z-<?php echo $k?>"><span class="glyphicon glyphicon-map-marker"></span>เพื่อน<?php echo $this->zone[$k]['n']?></a></h4>
<ul>
<?php
foreach($n as $v):?>
<li><a href="/p-<?php echo $v['_id']?>"><?php echo $v['t']?></a></li>
<?php endforeach?>
...
<a href="<?php echo self::uri(['boyz','/friend'])?>" style="color:#fff;">หาเพื่อน<strong style="color:#F60">เกย์</strong> สังคมชาวเกย์ เกย์คิง เกย์ควีน คลิกที่นี่</a>
</div>
<div style="padding:10px; text-align:center; font-size:16px; background:#000; margin:5px 0px 5px 0px; font-weight:bold;">
<a href="<?php echo self::uri(['lesbian','/friend'])?>" style="color:#fff;">หาเพื่อน<strong style="color:#F60">เลสเบี้ยน</strong> สังคมชาวเลสเบี้ยน ทอม ดี้ เลสคิง เลสควีน คลิกที่นี่</a>
</div>
<table class="table ins" width="100%">
<tr><td class="colum">อีเมลเฟสบุ๊ค์</td><td colspan="3"><input type="email" class="form-control" name="email" style="width:300px" required> <span class="h"> * บังคับกรอก</span><?php if($this->error['email']):?><div class="error"><?php echo $this->error['email']?></div><?php endif?></td></tr>
...
<tr>
<td class="colum">Facebook</td><td width="40%"><div class="input-group"><span class="input-group-addon">facebook.com/</span><input type="text" name="facebook" class="form-control" style="max-width:150px" readonly></div> <input type="hidden" name="facebook_id" id="facebook_id"><input type="hidden" name="facebook_name" id="facebook_name"><button class="btn btn-default" type="button" onClick="addfb()">เพิ่ม!</button></td>
