...
Load::Ajax()->register('newqueue');

Load::$core->data['title']='เพิ่ม - เบิกเงิน | '.Load::$core->data['title'];

Load::$core->data['content']=Load::$core->fetch('queue.insert');

function newqueue($par)
{
  $arg=[];
  ...
  if($arg['form']&&$arg['type']&&$arg['team'])
  {
    ...
    if($arg['type']!=1 || $arg['product'])
    {
      ...
      $id=Load::DB()->insert('team_queue',$arg);
      team::move('/queue/update/'.$id.'?completed');
      return;
    }
  }
