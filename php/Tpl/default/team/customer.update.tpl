...
<ul class="breadcrumb">
  ...
  <span class="divider">&raquo;</span>
	<li><a href="/customer/update/<?php echo $this->customer['_id']?>" title="">แก้ไข</a></li>
</ul>

<div class="box-white">
  <h3 class="bar-heading">แก้ไขบริษัท</h3>
  ...
  <?php endif?>
	<form action="<?php echo URL?>" class="form-horizontal" method="post" onsubmit="_.ajax.gourl('<?php echo URL?>','updatecustomer',this);return false;">
    ...
    <div class='box-body-add' style="padding-bottom:15px;border-bottom:1px solid #ccc;margin-bottom:10px">
      ...
      <?php if(is_array($this->customer['contact'])):?>
        ...
        <table class="table table-bordered table-hover">
          ...
          <?php for($i=0;$i<count($this->customer['contact']);$i++):$v=$this->customer['contact'][$i];?>
            <tr class="row-contact-<?php echo $i?>">
              ...
              <td><a href="javascript:;" class="btn btn-xs btn-danger btn-javascript-del" onClick="delRow(<?php echo $i?>);"><span class="glyphicon glyphicon-trash"></span></a></td>
            </tr>
          <?php endfor?>
          </tbody>
        </table>
    ...
    <div class='box-body'>
      <div class="form-group">
        ...
        <div><input type="file" name="attachment[]" id="attachment" value="เลือกไฟล์" multiple></div>
...
<script>
var by = "<?php echo $this->customer['by']?>";
var type = "<?php echo $this->customer['type']?>";
var parent = "<?php echo $this->customer['parent']?>";
var ID = '<?php echo $this->customer['_id']?>';

$(function () {
	_.upload.create('#attachment',function(){_.ajax.gourl('<?php echo URL?>','refreshfiles')});
	var config = {
		 '.chzn-select'           : {},
...
function newrowitem()
{
	...
	var contact ='<div class="g">'
							...
							+'  </div>'
							+'  <a href="javascript:;" class="btn btn-xs btn-danger btn-javascript-del" onClick="del(this);"><span class="glyphicon glyphicon-trash"></span> ลบผู้ติดต่อ</a>'
							+'  <hr style="clear:both;">'
							+'</div>';
...
function delRow(e)
{
  if(confirm("คุณต้องการลบผู้ติดต่อนี้หรือไม่"))
  {
    $.ajax({
      type: "POST",
      url: "/adminAccountingCustomers/data_del_contact",
      data: {contact_ID:e},
      dataType: 'json',
      //async: false,
      success: function(data){ $('.row-contact-'+e).remove();setNum(); }
    });
  }
}
...
{
  _.box.confirm({title:'ลบไฟล์',detail:'ต้องการลบไฟล์นี้หรือไม่',click:function(){_.ajax.gourl('<?php echo URL?>','delfile',link,ext)}});
}
</script>
