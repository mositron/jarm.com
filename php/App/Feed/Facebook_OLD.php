...
class Facebook
{
  public function __construct()
  {
    Load::$conf['tmp']['cur']=0;
    Load::$conf['tmp']['ads']=[];
    Load::$conf['tmp']['ads'][] = '<figure class="op-ad"><iframe width="300" height="250" style="border:0; margin:0;" src="https://www.facebook.com/adnw_request?placement=1579651482324810_1579651618991463&adtype=banner300x250"></iframe></figure>
    ';
    Load::$conf['tmp']['ads'][] = '<figure class="op-ad"><iframe width="300" height="250" style="border:0; margin:0;" src="https://www.facebook.com/adnw_request?placement=1579651482324810_1579661178990507&adtype=banner300x250"></iframe></figure>
    ';
    Load::$conf['tmp']['ads'][] = '<figure class="op-ad"><iframe width="300" height="250" style="border:0; margin:0;" src="https://www.facebook.com/adnw_request?placement=1579651482324810_1580998728856752&adtype=banner300x250"></iframe></figure>
    ';
    Load::$conf['tmp']['ads'][] = '<figure class="op-ad"><iframe width="300" height="250" style="border:0; margin:0;" src="https://www.facebook.com/adnw_request?placement=1579651482324810_1581010088855616&adtype=banner300x250"></iframe></figure>
    ';
    ...
    if($tmp=(new \Jarm\App\News\Service(['ignore'=>1]))->find($arg,['_id'=>1,'u'=>1,'t'=>1,'d'=>1,'sv'=>1,'fd'=>1,'c'=>1,'cs'=>1,'da'=>1,'ds'=>1,'di'=>1,'de'=>1,'exl'=>1,'url'=>1],['sort'=>$sort,'skip'=>$skip,'limit'=>$limit]))
    {
      ...
      foreach($tmp as $v)
      {
        ...
        for($i=0;$i<$lines;$i++)
        {
          $is_img=false;
          $txt=($i>0?$split:'').$spl[$i];
          $len=$this->charlen($txt);
          if(strpos($spl[$i],'<figure')!==false)
          {
            $is_img=true;
            $ads_img++;
            $ads_imgs++;
          ...
          elseif($len>0)
          {
            $ads_txt++;
            $ads_len+=$len;
            $ads_words+=$this->str_word_count_utf8($txt);
          }

          if(
            ($is_img && ($ads_index==0))// && ($ads_words>=100))
            ||
            ($is_img && ($ads_words>=350))
            ||
            ($is_img && ($ads_img>=5))
            ||
            ($i==$lines-1 && ($ads_words>=350))
            )
          {
        ...
        $ct=str_replace('<p-remove>','',implode($split,$spl));
        if($v['c']==28)
        {
          $ct=str_replace('<fb:ads>','',$ct);
        }
        else
        {
          $ct=preg_replace_callback('/\<fb\:ads\>/i',[$this,'call_ads'],$ct);
        }
  ...
  public function call_img($arg)
  {
    ...
    $txt.='<p-remove><figure data-feedback="fb:likes, fb:comments">
  <img src="'.$arg[4].'" />
  '.$em.'</figure>';
    return $txt;
  }
  ...
  public function call_youtube($txt)
  {
    ...
    {
      return '<p-remove><figure class="op-interactive"><iframe src="https://www.youtube.com/embed/'.$c[2].'" width="320" height="180"></iframe></figure>';
    }
    else
    {
  ...
  public function call_facebook($txt)
  {
    ...
    if(substr($link,0,4)=='http')
    {
      return '<p-remove><figure class="op-interactive"><iframe>
  ...
  public function call_facebook_video($txt)
  {
    ...
    if(substr($link,0,4)=='http')
    {
      return '<p-remove><figure class="op-interactive"><iframe src="https://www.facebook.com/plugins/video.php?href='.urlencode($link).'" width="320" height="180"></iframe></figure>';
    }
    else
    {
