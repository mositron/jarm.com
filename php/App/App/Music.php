...
class Music extends Service
{
  ...
  public function json_home()
  {
    ...
    $news=(new \Jarm\App\News\Service(['ignore'=>1]))->find(['pl'=>1,'c'=>24,'exl'=>0],[],['skip'=>0,'limit'=>5]);
  ...
  public function json_news_view()
  {
    ...
    $data=[
      'status'=>'ok',
      'template'=>Load::$core->assign('news',$news)->fetch('app/music.news.view')
    ];
    if($_GET['callback'])
    {
      header('Content-type: text/javascript');
