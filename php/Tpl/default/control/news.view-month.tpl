...
.bar{height:130px;position:relative;border-bottom:1px solid #000;}
.bar>div{width:30px;margin:0px auto;overflow:hidden;color:#ccc;border:1px solid #000;border-bottom:none;position:absolute;right:2px;bottom:0px;background:#52BBC3;text-align:center;}
...
<ul class="breadcrumb" style="margin-bottom:5px;">
  ...
  <span class="divider">&raquo;</span>
  <li><a href="/news/view-month/<?php echo self::$path[1]?>">อ่านข่าวประจำวันเดือน <?php echo self::Time()->month[intval(substr($this->date,4))-1].' '.(substr($this->date,0,4)+543)?></a></li>
</ul>
<?php
$sum=0;
$sum2=0;
...
<table class="table">
<tbody>
<?php for($i=0;$i<count($this->view);$i++):$v=$this->view[$i]?>
<tr>
...
<td class="u<?php echo $u?> score writer">
  ...
  <div class="l"><?php echo number_format($ur)?><span<?php if($i>0):$uro=intval($this->view[$i-1]['ur'][$u]);?> class="<?php echo $ur>$uro?'up':($ur<$uro?'down':'')?>"<?php endif?>></span></div>
</td>
<?php endforeach?>
...
<tr>
  ...
  <td class="u<?php echo $u?> bar writer" data-perc="<?php echo $p?>"><div style="height:<?php echo floor($p*95)+30?>px"><span><?php echo $id+1?></span></div></td>
  <?php endforeach?>
  <td></td>
</tr>
<tr>
...
<a href="/news/view-month">เดือนนี้</a><?php
for($i=intval(date('Ym'))-1;$i>=intval(date('Y').'01');$i--):
?>
, <a href="/news/view-month/<?php echo $i?>"><?php echo self::Time()->month[intval(substr($i,4))-1].' '.(substr($i,0,4)+543)?></a>
<?php endfor?>
