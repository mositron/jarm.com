...
<tr><td class="r">รูปภาพ/Flash</td><td>
...
<?php endif?>
<?php if($this->banner['l']):?><tr><td class="r">ลิ้งค์ปลายทาง</td><td><?php echo $this->banner['l']?></td></tr><?php endif?>
<?php if($this->banner['code']):?><tr><td class="r">โค๊ด</td><td><?php echo htmlspecialchars($this->banner['code'],ENT_QUOTES,'utf-8')?></td></tr><?php endif?>
<?php if($this->banner['d']):?><tr><td class="r">รายละเอียด</td><td><?php echo $this->banner['d']?></td></tr><?php endif?>
...
<?php foreach($this->banner[$k] as  $x=>$v):?>
<div>
 -  <?php echo $s['l'][$x]['t']?> :
<?php for($i=0;$i<count($v);$i++):?>
<?php echo $i?', ':''?><span class="bn bn-<?php echo $v[$i]?>"><?php echo strtoupper($v[$i])?></span>
<?php endfor?>
</div>
<?php endforeach?>
</div>
...
<div style="margin:10px">
    ...
    <h3 class="bar-heading">Impression</h3>
    <div class="row">
        <div class="col-sm-4">
    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            ...
            <?php else:?>
            <div><em>โฆษณานี้ใช้งานรูปแบบโค๊ด ทำให้ไม่สามารถนับจำนวนการคลิกได้.</em></div>
            <?php endif?>
        </div>
        <div class="col-sm-4">
            <h3 class="bar-heading">Top Devices</h3>
  ...
  <h3 class="bar-heading">Statistics</h3>
    ...
    <th class="text-right" width="120">CTR</th>
    </tr>
    </thead>
    <tbody>
    ...
    <td class="text-right"><?php echo number_format($imp=intval($v['imp']['do']))?></td>
    <td class="text-right"><?php echo number_format($click=intval($v['click']['do']))?></td>
    <td class="text-right"><?php echo number_format(($click/max(1,$imp))*100,3)?>%</td>
    </tr>
    <?php endfor?>
    </tbody>
    </table>
