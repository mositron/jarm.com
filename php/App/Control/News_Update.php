  ...
  public function get($parent,$id)
  {
    ...
    if($_POST)
    {
      if(Load::$my['am'] || (($parent->news['u']==Load::$my['_id']) && ((!$parent->news['ds']) || (Load::Time()->sec($parent->news['ds']) > time()-(3600*24)))))
      {
        ...
        if(defined('ADS_CONTROL'))
        {
          $arg['na']=($_POST['no_ads']?1:0);
        }
        ...
        if(!count($error))
        {
          ...
          if(stripos($arg['d'],'[embed-')!==false)
          {
            $arg['line']=0;
          }
          elseif(stripos($arg['d'],'<iframe')!==false)
          {
            $arg['line']=0;
          }
          else
...
*/
          ...
          if((!in_array($arg['c'],[11,12,28,32])) && ($parent->news['_id']>53815) && !$arg['exl'])
          {
            if($arg['pl'])
            {
