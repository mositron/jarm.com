      ...
      <div class="text-left">
        <table class="table table-hover">
          ...
          <tbody>
            ...
            <tr>
              <td><a href="/queue/<?php echo $v['_id']?>"><span class="label label-primary"><?php echo self::Time()->from($v['dp'],'datetime')?></span></a></td>
              <td><a href="/queue/<?php echo $v['_id']?>"><?php echo $v['title']?></a><p>สร้างโดย <a href="/user/<?php echo $v['u']?>" title=""><?php echo $this->people[$v['u']]['th']['first'].' '.$this->people[$v['u']]['th']['last']?></a></p></td>
              <td>
                <?php foreach((array)$v['ref'] as $k2=>$v2):$p=$this->people[$v2];?>
                <a href="/user/<?php echo $p['_id']?>" title="<?php echo $p['th']['first'].' '.$p['th']['last']?>"><img class="img-circle" src="http:/f1.jarm.com/team/user/<?php echo $p['_id']?>-s.jpg" style="width:32px;"></a>
                <?php endforeach?>
              </td>
              <td class="w50">
